...
''' <summary>
''' Used by the extension 'Type.GetExtensionMethods()' (see File 'TypeExtensions.vb)'
''' </summary>
Friend Class ExtensionMethodCache
  Implements IEnumerable(Of MethodInfo)

#Region " Static Part "

  Private Shared _Instances As New Dictionary(Of Type, ExtensionMethodCache)
  Private Shared _AppDomainListenerEnabled As Boolean = False

  Public Shared Function OfType(Of TExtendee)() As ExtensionMethodCache
    Return OfType(GetType(TExtendee))
  End Function

  Public Shared Function OfType(tExtendee As Type) As ExtensionMethodCache
    ...
    Else
      Dim newInstance As New ExtensionMethodCache(tExtendee, AddressOf ExtensionMethodCache.OfType)
...
#Region " Declarations & Constructor "
  ...
  Private _InterfaceExtensionMethods As New Dictionary(Of Type, ExtensionMethodCache)
  Private _BaseTypeExtensionMethods As ExtensionMethodCache = Nothing

  Friend Sub New(extendeeType As Type, subCacheGetter As Func(Of Type, ExtensionMethodCache))
